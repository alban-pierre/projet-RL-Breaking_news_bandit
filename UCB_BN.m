function [rew, draws, hot_expected, hot_real] = UCB_BN(tmax, MAB)
    ...
    for t=NbArms+1:tmax
        if (hotState == 0) %If no arm is supposed hot
            ...
            if ((rew(1,t)-rmin)/(rmax-rmin) <= (max_rew(1,ima)-rmin)/(rmax-rmin)+sqrt(1./(2*na(1,ima)))) %The sample is in the range expected: arm not hot
                ...
                smu(1,ima) = rew(1,t);
                na(1,ima) = 1;
                max_rew(1,ima) = rew(1,t)+(rmin+(rmax-rmin)*sqrt(1./(2*na(1,ima))));
            ...
            [ma, ima] = max(mu+sqrt(log(t)./(2*na)), [], 2);
            [rew(1,t),h] = MAB.sample(ima);
            if (ima == hotState && (rew(1,t)-rmin)/(rmax-rmin) < (max_rew(1,ima)-rmin)/(rmax-rmin)-sqrt(1./(2*na(1,ima)))) % Leave hot state
                ...
                if (ima == hotState)
                    ...
                    max_rew(1,ima) = min(max_rew(1,ima), rew(1,t)+(rmin+(rmax-rmin)*sqrt(1./(2*na(1,ima)))));
