function [rew, draws] = NND_UCB(tmax, MAB, squeezed)

    % Decides which arm to draw based on neightbors of the space (x = last reward of the arm, y = time between now and the last time this arm drawn)
    % -> we compute the mean of the neighbors of the current point whithin a range of D in each arm, and choose the maximum of this mean + UCB variance
    % All points of the space are between 0 and (squeezed) for the reward axis, and between 0 and 1 for the time axis

    % Strangely it performs very bad although it is a refinement of KNN_UCB_NEW
...
%        else
            for i=1:NbArms
                % nearest neightbors algo
                % Here we take all neighbors that are whithin a range of d to compute the mean
                mu(1,i) = nnd(rw{i}, 2*exp(-tl{i}+1), 2*exp(-ta(1,i)+1), 0.05);
            end
