...
\begin{abstract}
Multi-armed bandit is a very common problems, studied with many different approachs.
...
The Multi-armed Bandit (MAB) is a very important problem in the of Reinforcement Learning.
...
arm while it is hot. When no arms is hot, the algorithm should pull each arm frequently
in order to find a new breaking new. But during that time, the algorithm should
also maximize the reward, because each arm does not hove the same 'normal' reward.
...
several algorithms fitting the Breacking-news MAB problem.
